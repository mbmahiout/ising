...
class IsingEval: 
    def __init__(
        self, true_model, est_models, true_sample, est_samples, labels, layout_spec, analysis_path=None, metadata=None
    ):
        """
    ...
    def generate_plots(self):       
        ...
        display(VBox([self.fig, HBox([self.save_button_figure, self.save_button_results, self.save_button_metadata])]))
    ...
    def plot_scatter(self, ftr_name, row, col, pad=0.5):
        ...
        for est_data, label, color in zip(est_datas, self.labels, colors):
            IsingEval._add_comparison_scatter(self.fig, true_data, est_data, color, label, row, col)
    ...
    def plot_histogram(self, ftr_name, row, col):
        ...
        for est_data, label, color in zip(est_datas, self.labels, colors):
            ...
            IsingEval._add_histogram(self.fig, counts, bin_centers, bin_width, label, color, row, col)
        counts, _ = np.histogram(true_data, bins=bin_edges, density=True)
        IsingEval._add_histogram(self.fig, counts, bin_centers, bin_width, label="True", color="grey", row=row, col=col)
        ...
        for (ftr_name, plot_type), (row, col) in self.layout_spec.items():
            if plot_type == "scatter":
                scatter_results[f'{ftr_name} - ({row, col})'] = {
                    ...
                    for label, model, sample in zip(self.labels, self.est_models, self.est_samples)
                }
    ...
    def get_hist_results(self):
        ...
        for (ftr_name, plot_type), (row, col) in self.layout_spec.items():
            if plot_type == "histogram":
                hist_results[f'{ftr_name} - ({row, col})'] = self.get_ks_test_results(ftr_name)
        return hist_results
    ...
    def get_ks_test_results(self, ftr_name):
        ...
        for m1, s1, l1 in zip(range(num_models), range(num_samples), range(num_labels)):
            for m2, s2, l2 in zip(range(m1 + 1, num_models), range(s1 + 1, num_samples), range(l1 + 1, num_labels)):
    ...
    def increment_path_num(path):
        pattern = r"(.*?)(\d+)(\.\w+)$"
        return re.sub(pattern, lambda m: f"{m.group(1)}{int(m.group(2))+1}{m.group(3)}", path)